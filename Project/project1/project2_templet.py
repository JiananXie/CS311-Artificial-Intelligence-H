...
pos_weight = np.array([[800, -120, -50, -60, -60, -50, -120, 800],
                       [-120, -50, 1, 1, 1, 1, -50, -120],
                       [-50, 1, 3, 2, 2, 3, 1, -50],
                       [-60, 1, 2, 1, 1, 2, 1, -60],
                       [-60, 1, 2, 1, 1, 2, 1, -60],
                       [-50, 1, 3, 2, 2, 3, 1, -50],
                       [-120, -50, 1, 1, 1, 1, -50, -120],
                       [800, -120, -50, -60, -60, -50, -120, 800]])
    ...
    def evalutation(self, state):
        ...
        if self.is_terminal(state, self.color) or self.is_terminal(state, -self.color):
            if len(my_chess) < len(oppo_chess):
                return 1000 + pos_value() + -5 * move_value() - 30 * stable_value() - 35 * count_value()
            elif len(my_chess) > len(oppo_chess):
                return -1000 + pos_value() + -5 * move_value() - 30 * stable_value() - 35 * count_value()
            else:
                return pos_value() + -5 * move_value() - 30 * stable_value() - 35 * count_value()
        else:
